...
from Functions.auxiliary import format_query, generate_final_df, format_author_arxiv, format_author_googlescholar
...
def arxiv_search(query, max_results=10, year_low=2020, year_high=2025):
    ...
    for result in tqdm(results, desc="Fetching ArXiv articles"):  # Progress bar while fetching results
        papers.append({
            ...
            "Authors": [f'{author.name}' for author in result.authors],  # Combine author names
    ...
    papers["Authors"] = papers["Authors"].apply(format_author_arxiv)
...
def googleScholar_search(query, max_results=10, year_low=2020, year_high=2025):
    ...
    # Format authors correctly
    papers["Authors"] = papers["Authors"].apply(format_author_googlescholar)
...
def search_papers(query, max_results=10, year_low=2020, year_high=2025):
    ...
    try:
        ...
        pubmed = pd.DataFrame(columns=["DOI", "Title", "Abstract", "Year", "Authors", "PDF_URL", "Database"])
        print(f"Could not perform search in PubMed: {e}")
    ...
    try:
        ...
        google_scholar = pd.DataFrame(columns=["DOI", "Title", "Abstract", "Year", "Authors", "PDF_URL", "Database"])
        print(f"Could not perform search in Google Scholar: {e}")

    # Perform search on ArXiv
    try:
        arxiv = arxiv_search(query=query, max_results=max_results, year_low=year_low, year_high=year_high)
    except Exception as e:
        arxiv = pd.DataFrame(columns=["DOI", "Title", "Abstract", "Year", "Authors", "PDF_URL", "Database"])
        print(f"Could not perform search in ArXiv: {e}")
